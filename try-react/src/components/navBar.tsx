...
const NavBar = () => {
  ...
  const handleMenuNavigation = (path:string) => {
    if (needsUpdate) { errorAlert('נא לעדכן את הפרטים שלך לפני שתמשיך'); 
      return; }
console.log("i will go to",path);

    navigate(path);
    console.log(path,'go to:');
        ...
        {isMobile && (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <Tooltip title="התראות" arrow>
              <IconButton color="inherit" sx={{color: 'white', mr: 1,
            ...
            <Menu
              ...
              <MenuItem onClick={() =>{
                 handleMenuNavigation('/dashboard')}} dense>
                <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', direction: 'rtl' }}>
